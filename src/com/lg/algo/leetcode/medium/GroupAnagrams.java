package com.lg.algo.leetcode.medium;
...
//        System.out.println(lists);
    ...
    public List<List<String>> groupAnagrams(String[] strs) { //Time Limit Exceeded
        List<List<String>> result = new ArrayList<>();
        ...
        for (int i = 0; i < strs.length; i++) {
            ...
            if (!all.contains(strs[i])) anagrams.add(strs[i]);

            for (int j = i + 1; j < strs.length; j++) {
                if (!all.contains(strs[j]) && isAnagram(strs[i], strs[j])) {
                    anagrams.add(strs[j]);
                }
            }
            all.addAll(anagrams);
            if (anagrams.size() != 0) result.add(anagrams);
        }
